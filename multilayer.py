...
class optimizers():
    ...
    @staticmethod
    def RMSprop(index, gamma, dw, db, sdw, sdb, learning_rate):
        sdw = gamma * sdw + (1 - gamma) * dw ** 2
        sdb = gamma * sdb + (1 - gamma) * db ** 2

        model.layers[index].weight -= (learning_rate / (np.sqrt(sdw + 1e-08))) * dw
        model.layers[index].bias -= (learning_rate / (np.sqrt(sdb + 1e-08))) * db

    @staticmethod
    def Adam(index, gamma1, gamma2, dw, db, vdw, vdb, sdw, sdb, currentepoch, learning_rate):
        vdw = gamma1 * vdw + (1 - gamma1) * dw
        vdb = gamma1 * vdb + (1 - gamma1) * db

        sdw = gamma2 * sdw + (1 - gamma2) * dw ** 2
        sdb = gamma2 * sdb + (1 - gamma2) * db ** 2

        vdw_corr = vdw / (1 - np.power(gamma1, currentepoch + 1))
        vdb_corr = vdb / (1 - np.power(gamma1, currentepoch + 1))

        sdw_corr = sdw / (1 - np.power(gamma2, currentepoch + 1))
        sdb_corr = sdb / (1 - np.power(gamma2, currentepoch + 1))

        model.layers[index].weight -= (learning_rate / (np.sqrt(sdw_corr + 1e-08))) * vdw_corr
        model.layers[index].bias -= (learning_rate / (np.sqrt(sdb_corr + 1e-08))) * vdb_corr
